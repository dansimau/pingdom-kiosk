...
//TODO: this fails hard when the server is down/whatever
	var self = this;
...
Monitor.prototype.monitorStatus = function(callback) {
	...
	for (var i = 0, l = this.connectors.length; i < l; i++) {
		var account = {'username':this.connectors[i].options.username, 'status':this.connectors[i].options.status};
	...
	for (check in this._checkStatuses) {
		if (this._checkStatuses[check].status == 'up') {
			upChecks++;
		}else if (this._checkStatuses[check].status == 'down') {
			downChecks++;
		}
	...
	ret.checks = {
		'total': this._checkStatuses.length || 0,
		'up': upChecks,
